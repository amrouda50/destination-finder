...
export class HyperparameterTuning {
    ...
    async generateParameterCombinations(algorithmType) {
        if (!this.parameterSpaces[algorithmType]) return [{}];
            ...
            for (let i = 0; i < combinations.length; i += this.config.batchSize) {
                ...
                batch.forEach(combo => {
                    values.forEach(value => {
                        ...
                        const comboString = JSON.stringify(newCombo);
                        if (!uniqueCombinations.has(comboString)) {
                            uniqueCombinations.add(comboString);
    ...
    transformParametersToStoreStructure(params, algorithmType) {
        ...
        //     transformed[algorithmType.toLowerCase()].distanceDecay = {
        //         ...baseParams[algorithmType.toLowerCase()].distanceDecay,
        //         ...params.distanceDecay
        //     };
        // }
    
        // if (params.weekAllocation) {
        //     transformed[algorithmType.toLowerCase()].weekAllocation = {
        //         ...baseParams[algorithmType.toLowerCase()].weekAllocation,
